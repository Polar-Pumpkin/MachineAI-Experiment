...
class HighResolutionModule(nn.Module):
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        # 由于目前发现 block.expansion 恒为 1
        # 所以可以认为输入通道等于输出通道
        # 并且 4 个 BasicBlock 的输入输出通道都是一样的

        # 如果后期 block.expansion 出现变动
        # 这里需要考虑的东西就多啦
        # nn.Sequential(*layers)
        return DenseConnectionBlock(layers, self.num_inchannels[branch_index])
...
def hrnet_classification(pretrained=False, backbone='hrnetv2_w18'):
    model = HighResolutionNet_Classification(
        num_classes=1000, backbone=backbone)
    if pretrained:
        model_urls = {

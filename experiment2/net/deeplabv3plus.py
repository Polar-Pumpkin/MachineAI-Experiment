...
class ASPP(nn.Module):
    def __init__(self, dim_in, dim_out, rate=1, momentum=0.1):
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, num_classes: int, backbone: str = 'mobilenet', downsample_factor: int = 16,
        ...
        # ASPP特征提取模块
        # 利用不同膨胀率的膨胀卷积进行特征提取
        self.aspp = ASPP(dim_in=input_channels, dim_out=256, rate=16 // downsample_factor)
    ...
    def forward(self, x):
        ...
        x = self.cls_conv(x)
        x = functional.interpolate(x, size=(H, W), mode='bilinear', align_corners=True)
        return x
